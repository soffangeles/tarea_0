...
def validar():
    ...
    archivo_alumno = archivos[0]
    spec = importlib.util.spec_from_file_location("modulo_alumno", archivo_alumno)
    modulo = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(modulo)

    for variable in ["NOMBRE", "NUMERO_CUENTA", "CORREO", "CARRERA", "SEMESTRE"]:
        assert hasattr(modulo, variable), f"Error: Falta definir la variable {variable}"

    cuenta_limpia = str(modulo.NUMERO_CUENTA).strip().replace("-", "")
    cuenta_hash = hashlib.sha256(cuenta_limpia.encode("utf-8")).hexdigest()

    assert cuenta_hash in HASHES_VALIDOS, "Error: El número de cuenta no coincide con la lista oficial."
    assert len(str(modulo.NOMBRE).strip()) >= 5, "Error: Nombre demasiado corto."
    assert "@" in str(modulo.CORREO) and "." in str(modulo.CORREO), "Error: Formato de correo inválido."
    assert isinstance(modulo.SEMESTRE, int) and modulo.SEMESTRE > 0, "Error: Semestre inválido."
